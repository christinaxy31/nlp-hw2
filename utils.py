...
def beam_search_decode(model, src, src_mask, max_len, start_symbol, beam_size, end_idx):
    ...
    for i in range(max_len - 1):
        ...
        # get top-k scores from the flattened `scores` (vocab_size x beam_size)
        scores, indices = torch.topk(scores.reshape(-1), beam_size)
        # get the beam indices of the top-k scores
        beam_indices  = torch.divide(indices, vocab_size, rounding_mode='floor') # indices // vocab_size
        # get the indices in the vocab for each beam
        ...
        for beam_index, token_index in zip(beam_indices, token_indices):
            ...
            if prev_decoder_input[-1]==end_idx:
                token_index = end_idx # once EOS, always EOS
